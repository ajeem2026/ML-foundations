...
state_dict = torch.load('slp.pt', weights_only=True)
in_dim = state_dict['classifier.0.weight'].shape[1]
out_dim = state_dict['classifier.0.weight'].shape[0]

print(f"Loaded network dimensions: in={in_dim}, out={out_dim}")
...
def test(classifier=classifier, loss_fn=loss_fn):
    ...
    for actual, pred in zip(all_targets, all_preds):
        conf[actual][pred] += 1
